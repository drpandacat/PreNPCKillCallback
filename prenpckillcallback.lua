...
PRE_NPC_KILL.Internal.CallbackEntries = {
    {
        ...
        function (_, npc)
            ...
            data.____PRE_NPC_KILL_STORED_DAMAGE = 0
            data.____PRE_NPC_KILL_SKIP_NEXT = nil
        end,
    },
    {
        ...
        function (_, entity, amt, flags, source, delay)
            ...
            if npc.HitPoints - data.____PRE_NPC_KILL_STORED_DAMAGE > 0
            or npc:HasEntityFlags(EntityFlag.FLAG_NO_DEATH_TRIGGER)
            ...
            or (source.Type == EntityType.ENTITY_TEAR and source.Entity:ToTear():HasTearFlags(TearFlags.TEAR_ICE)) then return end

            for _, v in ipairs(Isaac.GetCallbacks(PRE_NPC_KILL.ID)) do
                if not v.Param or v.Param == npc.Type then
                    local result = v.Function(v.Mod, npc, source, amt, flags, delay)
